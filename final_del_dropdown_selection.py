...
def delete_row(partnumber, db_cursor, popup):
    try:
        with db_cursor:
            db_cursor.execute("DELETE FROM finish_goods_dropdown WHERE finish_goods = %s", (partnumber,))
...
def fetch_partnumbers(db_cursor):
    try:
        db_cursor.execute("SELECT partnumber FROM finish_goods_dropdown")
...
def final_delete_part_numbers(root):
    popup = tk.Toplevel(root)
    popup.title("Delete Finish Dropdown")
    ...
    try:
        ...
        add_image = "raw_remove_table.png"
        full_path = image_path + add_image
        # Load and display the same image
