...
def raw_add_part_number(entry_fields, password_entry, logged_in_user, popup):
    db_connection = None
    db_cursor = None
    try:
        ...
        query_iqc_storage = '''
            INSERT INTO raw_partnumbers (partnumber, "logged_in_users")
            VALUES (%s, %s)
        '''
...
def raw_add_partnumber_data(root, logged_in_user):
    # Set up the Tkinter window
    popup = tk.Toplevel(root)
    popup.title("Raw Dropdown Add List")

    popup.iconbitmap(icon_path)
    

    def validate_entry(text):
        # Check if the input text length is less than or equal to 4
        return len(text) <= 5
    ...
    submit_button = ttk.Button(popup, text="Submit", command=lambda: raw_add_part_number(entry_fields, password_entry, logged_in_user, popup))
