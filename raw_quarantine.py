...
from image_icon_paths import icon_path, passwords

db_password = passwords

def quarantine_raw(root, logged_in_user, db_cursor, db_connection):
    try:
        # Create a new Toplevel window
        data_popup = tk.Toplevel(root)
        data_popup.title("Quarantine table scrap or return to good stock.")

        data_popup.iconbitmap(icon_path)
        
        # Create a Treeview widget with the updated columns
        tree = ttk.Treeview(data_popup, columns=("Part Number", "Purchase Number", "Quantity", "Location", "User Account", "Date and Time"), show="headings")
        tree.pack(fill=tk.BOTH, expand=True)

        # Set up column headings with centered text
        for col in ("Part Number", "Purchase Number", "Quantity", "Location", "User Account", "Date and Time"):
        ...
        def populate_table():
            try:
                ...
                db_cursor.execute('SELECT "part_number", "purchase_number", "qty", "storage", "logged_in_users", to_char("time_stamp", \'DD-MM-YYYY    -    HH24:MI:SS\') FROM quarantine;')
        ...
        def export_to_excel():
            try:
                ...
                df = pd.DataFrame(data, columns=["Part Number", "Purchase Number", "Quantity", "Location", "User Account", "Date and Time"])
            ...
            try:
                ...
                def update_data():
                    try:
                        ...
                        edited_location = location_entry.get().upper()
                        ...
                        db_cursor.execute('''
                            INSERT INTO raw_scrap (part_number, purchase_number, qty, storage, logged_in_users) 
                            VALUES (%s, %s, %s, %s, %s)
                            ''', 
                        ...
                        purchase_number_fail_iqc = f"{selected_data[1]} -Quarantine Scrap"
                        ...
                        db_cursor.execute('DELETE FROM quarantine WHERE to_char("time_stamp", \'DD-MM-YYYY    -    HH24:MI:SS\') = %s', (time_stamp,))
                        db_connection.commit()
                        ...
                        messagebox.showinfo("Update Successful", "Data updated successfully. Part is scrap.")
                ...
                update_button = ttk.Button(edit_window, text="Scrap", command=update_data)
                update_button.grid(row=4, column=0, columnspan=2, padx=5, pady=10)

            except IndexError:
                messagebox.showwarning("No Selection", "Please select an item to edit.")

        # Function to handle returning selected data to stock
        def return_to_stock():
            try:
                # Get selected item
                selected_item = tree.selection()[0]
                ...
                table_select = 'raw_stock'

                first_name, last_name = logged_in_user.split(":")[1].strip().split(" ")

                # Display data for returning to stock
                return_window = tk.Toplevel(root)
                return_window.title("Return to Stock")

                return_window.iconbitmap(icon_path)
                ...
                # Display selected data for reference
                for idx, col_name in enumerate(("Part Number", "Purchase Number", "Quantity", "Location:")):
                    label = ttk.Label(return_window, text=col_name)
                    if col_name == "Location:":
                        label.config(font=("TkDefaultFont", 10, "bold"))  # Set the label to bold for "Location"
                    label.grid(row=idx, column=0, padx=5, pady=5, sticky="e")

                    entry = ttk.Entry(return_window)

                    # Check if the current column is for 'Location'
                    if col_name == "Location:":
                        # Add validation here for 'Location' entry
                        validate_command = (return_window.register(validate_entry), '%P')
                        location_entry = ttk.Entry(return_window, validate="key", validatecommand=validate_command)
                        location_entry.insert(0, selected_data[idx])
                        location_entry.grid(row=idx, column=1, padx=5, pady=5, sticky="w")  # Allow editing for Location
                    else:
                        entry.insert(0, selected_data[idx])
                        entry.config(state="disabled")  # Disable editing for other columns
                ...
                def update_data():
                    try:
                        ...
                        # Save data to raw_history table
                        db_cursor.execute('''
                            INSERT INTO raw_history (part_number, purchase_number, qty, storage, logged_in_users) 
                            VALUES (%s, %s, %s, %s, %s)
                            ''', 
                            (selected_data[0], f"{selected_data[1]} - Good parts return to IQC", selected_data[2], selected_data[3], f"{first_name} {last_name}"))
                        db_connection.commit()

                        # Delete the selected row from raw_scrap based on time_stamp
                        db_cursor.execute('DELETE FROM quarantine WHERE to_char("time_stamp", \'DD-MM-YYYY    -    HH24:MI:SS\') = %s', (time_stamp,))
                        db_connection.commit()
                        ...
                        messagebox.showinfo("Update Successful", "Data updated successfully. Part returned to stock.")
                ...
                # Button to update data
                update_button = ttk.Button(return_window, text="Return to Stock", command=update_data)
                update_button.grid(row=4, column=0, columnspan=2, padx=5, pady=10)

            except IndexError:
                messagebox.showwarning("No Selection", "Please select an item to return to stock.")
        ...
        edit_button = ttk.Button(frame, text="Scrap", command=edit_selected_data)
        edit_button.grid(row=0, column=2, padx=4, pady=1, sticky="nsew")

        # Create a button to return selected data to stock
        return_button = ttk.Button(frame, text="Return to Stock", command=return_to_stock)
        return_button.grid(row=0, column=3, padx=4, pady=1, sticky="nsew")
